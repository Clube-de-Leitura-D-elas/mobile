...
void main() {
  ...
  testWidgets('context.showToast exibe a mensagem e usa o type informado', (
    tester,
  ) async {
  ...
  testWidgets(
    'context.showToast usa AppToastType.info como padrão quando type não é informado',
    (tester) async {
      await tester.pumpWidget(
        wrap((context) {
          return ElevatedButton(
  ...
  testWidgets('context.showSuccessToast usa AppToastType.success', (
    tester,
  ) async {
    await tester.pumpWidget(
      wrap((context) {
  ...
  testWidgets('context.showErrorToast usa AppToastType.error', (tester) async {
    await tester.pumpWidget(
      wrap((context) {
        return ElevatedButton(
          onPressed: () =>
              context.showErrorToast('Não foi possível salvar os dados.'),
          child: const Text('disparar'),
        );
      }),
    );
  ...
  testWidgets('context.showWarningToast usa AppToastType.warning', (
    tester,
  ) async {
    await tester.pumpWidget(
      wrap((context) {
  ...
  testWidgets('respeita a duration customizada e remove o toast depois dela', (
    tester,
  ) async {
    await tester.pumpWidget(
      wrap((context) {
  ...
  testWidgets('usa 3 segundos como duration padrão quando não informada', (
    tester,
  ) async {
    await tester.pumpWidget(
      wrap((context) {
    ...
    expect(
      find.text('Padrão de duração'),
      findsOneWidget,
      reason: 'ainda não deveria ter sumido antes dos 3s',
    );
