...
void main() {
  group('AppTheme', () {
    test(
      'AppTheme.light configures light brightness, color tokens, and textTheme',
      () {
        final theme = AppTheme.light;
        const colorTokens = AppColorTokens.light;
        const typographyTokens = AppTypographyTokens.standard;
        ...
        expect(
          ...
          equals(typographyTokens.display.fontSize),
        );
        expect(
          theme.textTheme.displayLarge?.height,
          equals(typographyTokens.display.height),
        );
        expect(
          theme.textTheme.headlineLarge?.fontSize,
          equals(typographyTokens.headingH1.fontSize),
        );
        expect(
          theme.textTheme.bodyMedium?.fontSize,
          equals(typographyTokens.bodyDefault.fontSize),
        );
        expect(
          theme.textTheme.labelLarge?.fontSize,
          equals(typographyTokens.labelButton.fontSize),
        );

        final extensionColorTokens = theme.extension<AppColorTokens>();
        expect(extensionColorTokens, equals(colorTokens));

        final extensionTypographyTokens = theme
    ...
    test(
      'AppTheme.dark configures dark brightness, color tokens, and textTheme',
      () {
